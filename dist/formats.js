...
function format(id) {
    ...
    if (!match)
        throw new Error(`Missing audio format registry entry: ${id}`);
    return match;
}
...
export function detectAudioFormat(bytes) {
    if (startsWith(bytes, [0x66, 0x4c, 0x61, 0x43]))
        return format("flac"); // fLaC
    if (startsWith(bytes, [0x4f, 0x67, 0x67, 0x53]))
        return format("ogg"); // OggS
    ...
    if (startsWith(bytes, [0x66, 0x74, 0x79, 0x70], 4))
        return format("m4a"); // ISO BMFF
    const isWebm = startsWith(bytes, [0x1a, 0x45, 0xdf, 0xa3]) && includesAscii(bytes, "webm");
    if (isWebm)
        return format("webm");
    if (startsWith(bytes, [0x49, 0x44, 0x33]))
        return format("mp3"); // ID3
    if (bytes.length >= 2 && bytes[0] === 0xff) {
        ...
        const isMpegAudio = (second & 0xe0) === 0xe0 && version !== 0b01 && layer !== 0b00;
        if (isMpegAudio)
            return format("mp3");
    }
    return null;
}
export function findAudioFormatByExtension(fileName) {
    const extension = fileName.trim().split(".").at(-1)?.toLowerCase();
    if (!extension || extension === fileName.trim().toLowerCase())
        return null;
    return (AUDIO_FORMATS.find((candidate) => candidate.extensions.includes(extension)) ?? null);
}
export function findAudioFormatByMimeType(mimeType) {
    const normalized = mimeType.trim().toLowerCase().split(";", 1)[0];
    if (!normalized)
        return null;
    return (AUDIO_FORMATS.find((candidate) => candidate.mimeTypes.includes(normalized)) ?? null);
}
